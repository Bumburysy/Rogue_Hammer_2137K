...
public class LevelCompleteScene implements Scene {
    ...
    public LevelCompleteScene(SceneManager sceneManager, GameScene parentScene, Player player, GameStatistics stats) {
    ...
    public LevelCompleteScene(SceneManager sceneManager, GameScene parentScene, Player player) {
        this(sceneManager, parentScene, player, null);
    }
    ...
    public void create() {
        ...
        showCursor();
        SoundManager.playMusic(SoundManager.musicWin, true);
    }
    ...
    public void resize(int width, int height) {
        ...
        if (parentScene != null) {
            parentScene.resize(width, height);
        }
    }
    ...
    private void loadTextures() {
        titleTexture = new Texture("ui/complete.png");
        continueTexture = new Texture("ui/continue.png");
    ...
    private void calculateButtonBounds() {
        ...
        layout.advance(11);
        layout.advance();
        continueBounds = createCenteredButtonBounds(continueTexture, centerX, layout.getCurrentY());
        layout.advance();
    ...
    private void updateHover() {
        ...
        continueHovered = continueBounds.contains(mouse.x, mouse.y);
    ...
    private void handleInput() {
        if (Gdx.input.isButtonJustPressed(Input.Buttons.LEFT)) {
            if (continueHovered) {
                ...
                if (player != null) {
                    player.resetLevelTransitionRequest();
                    parentScene.startNewLevel(player);
                }
                return;
            }
            if (quitHovered) {
    ...
    private void renderStatistics(SpriteBatch batch) {
        ...
        drawCenteredText(batch, "LEVEL STATISTICS", centerX, layout.getCurrentY());
        layout.advance();
        ...
        drawCenteredText(batch, "Coins: " + (player != null ? player.getCoins() : 0), centerX, layout.getCurrentY());
        layout.advance();

        drawCenteredText(batch, "Keys: " + (player != null ? player.getKeys() : 0), centerX, layout.getCurrentY());
        layout.advance();

        drawCenteredText(batch, "Health: " + (player != null ? player.getHealth() : 0)
                + "/" + (player != null ? player.getMaxHealth() : 0), centerX, layout.getCurrentY());
        layout.advance();
    ...
    private void renderButtons(SpriteBatch batch) {
        renderButton(batch, continueTexture, continueBounds, continueHovered);
